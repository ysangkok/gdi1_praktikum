...
import testpackage.shared.ship.Ship.Direction;
...
public class Level implements Serializable {
	...
	static void checkShips(int player, Character[][] b, boolean countShips) throws InvalidLevelException {
	    ...
	    for (Ship s : ships) {
	    	...
	    	if (s.o == Orientation.HORIZONTAL) { // !alongxaxis
				debug(Util.format("west: trying to check %d,%d",s.x,s.y-1));
				if (s.y-1 >= 0 && staticIsShip(s.x,s.y-1,b)) {
					throw new InvalidLevelException(Util.format("ship north of %d,%d",s.x,s.y-1));
				}
	    	} else if (s.o == Orientation.VERTICAL) {
				debug(Util.format("north: trying to check %d,%d",s.x-1,s.y));
				if (s.x-1 >= 0 && staticIsShip(s.x-1,s.y,b)) {
					throw new InvalidLevelException(Util.format("ship north of %d,%d",s.x-1,s.y));
				}	    		
	    	}
	    	
	    	{
	    		...
	    		for (Integer[] arr : s.getAllOccupiedCoords()) {
	    			...
	    			if (s.o == Orientation.HORIZONTAL) {
	    				if (i>=1			&&	staticIsShip(i-1, j, b)) throw new InvalidLevelException(Util.format("There is a ship at (%d,%d) too close north of this ship: %s",i-1, j, s.toString())); 
	    				if (i<b.length-1	&&	staticIsShip(i+1, j, b)) throw new InvalidLevelException(Util.format("There is a ship at (%d,%d) too close south of this ship: %s",i+1, j, s.toString()));
	    			} else if (s.o == Orientation.VERTICAL) {
	    				if (j<b[0].length-1	&&	staticIsShip(i, j+1, b)) throw new InvalidLevelException(Util.format("There is a ship at (%d,%d) too close east of this ship: %s", i, j+1, s.toString())); 
	    				if (j>=1			&&	staticIsShip(i, j-1, b)) throw new InvalidLevelException(Util.format("There is a ship at (%d,%d) too close west of this ship: %s", i, j-1, s.toString()));
	    			}
	    		}
	    		...
	    		if (s.o == Orientation.HORIZONTAL) {
	    			debug(Util.format("east: trying to check %d,%d",s.x,j));
	    			if (j < b[s.x].length && staticIsShip(s.x,j,b)) {
	    				throw new InvalidLevelException(Util.format("ship east of %d,%d",s.x,j));
	    			}

	    		} else if (s.o == Orientation.VERTICAL) {
	    			debug(Util.format("south: trying to check %d,%d",i,s.y));
	    			if (i < b.length && staticIsShip(i,s.y,b)) {
	    				throw new InvalidLevelException(Util.format("ship south of %d,%d",i,s.y));
	    			}
	    		}
	    	}
	    }
	...
	private static int countShipLength(int x, int y, Direction d, Character[][] b, int akku) throws InvalidLevelException {
		if (d == Direction.DOWN) {
			...
			if (!matchChar("lLhH", b[x][y])) throw new InvalidLevelException(Util.format("Expected ship at (%d,%d) direction %s",x,y,d));
			try {
